...
const REFERRAL_MESSAGE = `Prom Connect – IIMA's anonymous matchmaking for Prom 2026! 💫 Find your date. Campus-only, privacy-first. Join at ${APP_URL}`;
...
export function sharePartnerInviteViaWhatsApp(fromName: string, inviterEmail: string): void {
  ...
  const message = `Hey! ${fromName} wants to go to Prom with you 💫\n\nThey're on Prom Connect – IIMA's anonymous matchmaking for Prom 2026. Join and accept their request:\n${inviteUrl}`;
